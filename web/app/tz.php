...
// 服务器状态检查（使用缓存）
$cacheFile = dirname(__DIR__) . '/logs/serverStatusCache.json';
$isAppRunning = false;
$useCache = false;
...
// 如果缓存无效，执行状态检查
if (!$useCache) {
    $output = shell_exec("ps aux | grep '[b]in/x64/factorio.*--start-server' 2>/dev/null");
    $isAppRunning = !empty(trim($output));
