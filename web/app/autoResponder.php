...
// 配置文件路径
define('SETTINGS_FILE', dirname(__DIR__) . '/config/state/chatSettings.json');
define('STATE_FILE', dirname(__DIR__) . '/config/state/autoResponderState.json');
define('LOG_FILE', dirname(__DIR__, 2) . '/factorio-current.log');
define('MESSAGE_QUEUE_FILE', dirname(__DIR__) . '/config/state/messageQueue.json');
define('AUTO_RESPONDER_LOG', dirname(__DIR__) . '/logs/autoResponder.log');
define('RCON_CONFIG_FILE', dirname(__DIR__) . '/config/system/rcon.php');
...
$rconConfigFile = RCON_CONFIG_FILE;
if (file_exists($rconConfigFile)) {
    $rconConfigData = require $rconConfigFile;
    $rconConfig = $rconConfigData['default'] ?? $rconConfigData;
} else {
    $rconConfig = [
        'rcon_enabled' => false,
        'rcon_port' => 27015,
...
function isServerRunning() {
    // 直接检查Factorio进程是否存在
    $result = shell_exec("ps aux | grep 'factorio --start-server' | grep -v grep");
    return !empty($result);
}
...
function logMessage($message) {
    ...
    file_put_contents(AUTO_RESPONDER_LOG, $logEntry, FILE_APPEND | LOCK_EX);
}
