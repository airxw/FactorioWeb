    ...
    if ($action === 'delete' && !empty($rawFilename)) {
        ...
        if (!preg_match('/^[\w\x{4e00}-\x{9fa5}-]+$/u', $rawFilename)) {
            ...
            $data = [
                ...
                // RCON 设置
                'rcon' => [
                    'enabled' => !empty($_POST['rcon_enabled']),
                    'port' => (int)($_POST['rcon_port'] ?? 27015),
                    'password' => $truncate($_POST['rcon_password'] ?? ''),
                ],
...
<body>
    <div class="container">
        <div class="nav-links">
            <a href="public/pages/index.html">← 返回控制面板</a>
        </div>
        ...
        <div class="section admin-section">
            <h2>🔐 管理员功能</h2>
            <p style="margin-bottom: 10px;">敏感信息（用户名、密码、令牌）已集中存储，游戏密码可在此设置。</p>
            <div class="form-group">
                <label>Factorio.com 用户名</label>
                <div style="display: flex; align-items: stretch; gap: 12px;">
                    <span id="username" class="readonly-input" style="display: inline-flex; align-items: center; min-width: 600px; padding: 8px 12px; background: var(--input-bg); border: 1px solid var(--border-color); border-radius: 4px; color: var(--text-muted); cursor: not-allowed;"><?= htmlspecialchars($secrets['username'] ?? '(未设置)') ?></span>
                    <a href="secretsManager.php" style="display: inline-flex; align-items: center; padding: 8px 16px; background: var(--danger); color: white; text-decoration: none; border-radius: 4px; font-size: 14px; white-space: nowrap;">
                        管理敏感信息 →
                    </a>
                </div>
                <div class="note">此信息由管理员在敏感信息中配置</div>
            </div>
        </div>
        <?php endif; ?>
        ...
        <form method="post">
            ...
            <div class="section">
                ...
                <div class="button-group">
                    ...
                    <button type="button" class="btn-secondary" onclick="location.href='public/pages/index.html'">返回控制面板</button>
            ...
            <!-- RCON 设置 -->
            <?php
            $rconConfig = $config['rcon'] ?? [];
            $rconEnabled = $rconConfig['enabled'] ?? true;
            $rconPort = $rconConfig['port'] ?? 27015;
            $rconPassword = $rconConfig['password'] ?? '';
            if (empty($rconPassword)) {
                $rconPassword = bin2hex(random_bytes(16));
            }
            ?>
            <div class="section rcon-section">
                <h2>🔧 RCON 远程控制设置</h2>
                <div class="form-group checkbox-group">
                    <input type="checkbox" id="rcon_enabled" name="rcon_enabled" <?= $rconEnabled ? 'checked' : '' ?>>
                    <label for="rcon_enabled">启用 RCON 远程控制</label>
                </div>
                <div class="form-group">
                    <label for="rcon_port">RCON 端口</label>
                    <input type="number" id="rcon_port" name="rcon_port" value="<?= $rconPort ?>" min="1" max="65535">
                    <div class="note">默认 27015，请确保端口未被占用</div>
                </div>
                <div class="form-group">
                    <label for="rcon_password">RCON 密码</label>
                    <div style="display: flex; gap: 8px;">
                        <input type="text" id="rcon_password" name="rcon_password" maxlength="100"
                               value="<?= htmlspecialchars($rconPassword) ?>" style="flex: 1;">
                        <button type="button" class="btn-secondary" onclick="generateRconPassword()" title="生成新密码">🔄</button>
                    </div>
                    <div class="note">用于远程控制台连接，建议设置强密码</div>
                </div>
            </div>
            ...
            <div class="section">
                ...
                <div class="form-group">
                    <label for="tags">标签（回车添加，点击 × 删除）</label>
                    <div id="tags-container" class="tags-container">
                        <div id="tags-list" class="tags-list"></div>
                        <input type="text" id="tags-input" class="tags-input" placeholder="输入标签后按回车" maxlength="20">
                    </div>
                    <input type="hidden" id="tags" name="tags" value="<?= htmlspecialchars(implode(',', $config['tags'] ?? [])) ?>">
                    <div class="note">最多添加5个标签，每个标签最长20字符</div>
                </div>
                <div class="form-group">
    ...
    (function() {
        ...
        function initTags() {
            ...
            if (value) {
                tags = value.split(',').filter(t => t.trim()).map(t => t.trim());
            }
            renderTags();
        }
    ...
    function generateRconPassword() {
        ...
        document.getElementById('rcon_password').value = password;
    }
    </script>
</body>
</html>
