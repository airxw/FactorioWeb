...
function doGift() {
    ...
    const fd = new FormData();
    fd.append('cmd', `/c game.get_player("${n}").insert{name="${i}",count=${c}}`);
...
document.addEventListener('DOMContentLoaded', ()=>{
    ...
    window.controlServer = (act) => { 
        apiCall(act, new FormData(document.getElementById('startForm')))
            .then((response)=> { 
                window.__probe.startProbe();
                // 如果是启动操作且成功，立即更新状态为运行中
                if (act === 'start' && response && response.running) {
                        ...
                        fetch('/app/api.php?action=rcon_status&server=default')
                            ...
                            .then(data => {
                                ...
                                if (statusBadge) {
                                    ...
                                    statusBadge.className = data.running ? 'badge bg-success' : 'badge bg-secondary';
                                }
                            })
                            .catch(() => {});
                    }, 1000);
